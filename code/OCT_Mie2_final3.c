...
double	gf;              /* anisotropy [-] */
double	gb;              /* anisotropy [-] */
double	alpha_f;              /* anisotropy [-] */
double	alpha_b;              /* anisotropy [-] */
double	C;              /* anisotropy [-] */
double TT_use[361];
double TT_is_use[181];
...
time_t	now;
double	start_time, finish_time, temp_time, start_time_all, end_time_all, start_time_post, end_time_post, start_time_post1, end_time_post1; /* for clock() */
...
float 	alb_tt[Ntiss];
float 	C_tt[Ntiss];
float TT[Ntiss][361] = {{0}};
float TT_is[Ntiss][181] = {{0}};
...
int find_sample=0;
double max_TT=0, ran1=0,ran2=0,Sample=0,max_TT_is=0,x_old=0,y_old=0,z_old=0;
int iindex=0;
int num_detp=0;
int sam=0;
// double ss[1000000] = {0};
...
int nang = 361;
int nang_is = 181;
double MM[1024][1024] = {{0}};
double fit[1024][1024] = {{0}};
double fit_BDR[1024][1024] = {{0}};
...
//    printf("argc = %d\n",argc);
    ...
    for(int aa = (times-1)*num_iter+1;aa<=(times)*num_iter;aa++){
...
//			while ((rnd = RandomGen(1,0,NULL)) < 0.0);
			...
			uy	= uy0;
			 uz	= uz0;
//			uz  = 0.9991;
//			sintheta = sqrt(1.0 - uz*uz);
//			while ((rnd = RandomGen(1,0,NULL)) < 0.0);
//			cospsi = 2*round(rnd)-1;
//			ux = cospsi*sintheta;
		}
		else // use mcflag
		{ 
          ...
          // KE: chapter 4.3.3 in Zhao's thesis
            ...
            //  printf("%lf\n", p);
                 ...
                 forward - scattering */
                  ...
                  double theta_S = acos(costheta_S);
                  double theta_B = acos(costheta_B);
                  f_HG = TT_final1(gf,gb,alpha_f,alpha_b,C,theta_S);
                  f_B = TT_is_final1(gf,gb,alpha_f,alpha_b,C,theta_B);
                  L_temp = f_HG / (p * f_B + (1 - p) * f_HG); // KE: equal to equation 4.4 in Zhao's thesis
                  L_current *= L_temp;
                  /* Update trajectory */
                  ux = upx;
                  uy = upy;
...
for(temp_lin=0;temp_lin<samplePoints;temp_lin++){
        ...
        for(temp_lin2=0;temp_lin2<samplePoints;temp_lin2++){
            ...
            if(fit_temp[temp_lin2]<0) fit_temp[temp_lin2]=0;
...
// end_time_post = clock();
// time_min = (double)(end_time_post-start_time_post)/CLOCKS_PER_SEC/60;
// printf("Postprocessing1 Time  = %5.3f min\n",time_min);
// start_time_post = clock();
...
// end_time_post = clock();
// time_min = (double)(end_time_post-start_time_post)/CLOCKS_PER_SEC/60;
// printf("Postprocessing2 Time  = %5.3f min\n",time_min);
